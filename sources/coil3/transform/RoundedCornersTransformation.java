...
public final class RoundedCornersTransformation extends Transformation {
    ...
    public Object transform(Bitmap bitmap, Size size, Continuation<? super Bitmap> continuation) {
        long m8418calculateOutputSizenmZnaPc = m8418calculateOutputSizenmZnaPc(bitmap, size);
        int m8426getFirstimpl = IntPair.m8426getFirstimpl(m8418calculateOutputSizenmZnaPc);
        int m8427getSecondimpl = IntPair.m8427getSecondimpl(m8418calculateOutputSizenmZnaPc);
        Bitmap createBitmap = Bitmap.createBitmap(m8426getFirstimpl, m8427getSecondimpl, BitmapsKt.getSafeConfig(bitmap));
        Canvas canvas = new Canvas(createBitmap);
        Paint newScaledShaderPaint = TransformationsKt.newScaledShaderPaint(bitmap, m8426getFirstimpl, m8427getSecondimpl);
        float f = this.topLeft;
        float f2 = this.topRight;
        if (f == f2) {
            float f3 = this.bottomLeft;
            if (f2 == f3 && f3 == this.bottomRight) {
                canvas.drawRoundRect(0.0f, 0.0f, m8426getFirstimpl, m8427getSecondimpl, f, f, newScaledShaderPaint);
                return createBitmap;
            }
        }
        ...
        RectF rectF = new RectF(0.0f, 0.0f, m8426getFirstimpl, m8427getSecondimpl);
        Path path = new Path();
    ...
    private final long m8418calculateOutputSizenmZnaPc(Bitmap bitmap, Size size) {
        if (SizeKt.isOriginal(size)) {
            return IntPair.m8422constructorimpl(bitmap.getWidth(), bitmap.getHeight());
        }
        ...
        if ((component1 instanceof Dimension.Pixels) && (component2 instanceof Dimension.Pixels)) {
            return IntPair.m8422constructorimpl(((Dimension.Pixels) component1).m8417unboximpl(), ((Dimension.Pixels) component2).m8417unboximpl());
        }
        int width = bitmap.getWidth();
        int height = bitmap.getHeight();
        Dimension width2 = size.getWidth();
        int m8417unboximpl = width2 instanceof Dimension.Pixels ? ((Dimension.Pixels) width2).m8417unboximpl() : Integer.MIN_VALUE;
        Dimension height2 = size.getHeight();
        double computeSizeMultiplier = DecodeUtils.computeSizeMultiplier(width, height, m8417unboximpl, height2 instanceof Dimension.Pixels ? ((Dimension.Pixels) height2).m8417unboximpl() : Integer.MIN_VALUE, Scale.FILL);
        return IntPair.m8422constructorimpl(MathKt.roundToInt(bitmap.getWidth() * computeSizeMultiplier), MathKt.roundToInt(computeSizeMultiplier * bitmap.getHeight()));
    }
}
