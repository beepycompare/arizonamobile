...
public final class RoundedCornersTransformation extends Transformation {
    ...
    public Object transform(Bitmap bitmap, Size size, Continuation<? super Bitmap> continuation) {
        long m9281calculateOutputSizenmZnaPc = m9281calculateOutputSizenmZnaPc(bitmap, size);
        int m9289getFirstimpl = IntPair.m9289getFirstimpl(m9281calculateOutputSizenmZnaPc);
        int m9290getSecondimpl = IntPair.m9290getSecondimpl(m9281calculateOutputSizenmZnaPc);
        Bitmap createBitmap = Bitmap.createBitmap(m9289getFirstimpl, m9290getSecondimpl, BitmapsKt.getSafeConfig(bitmap));
        Canvas canvas = new Canvas(createBitmap);
        Paint newScaledShaderPaint = TransformationsKt.newScaledShaderPaint(bitmap, m9289getFirstimpl, m9290getSecondimpl);
        float f = this.topLeft;
        float f2 = this.topRight;
        if (f == f2) {
            float f3 = this.bottomLeft;
            if (f2 == f3 && f3 == this.bottomRight) {
                canvas.drawRoundRect(0.0f, 0.0f, m9289getFirstimpl, m9290getSecondimpl, f, f, newScaledShaderPaint);
                return createBitmap;
            }
        }
        ...
        RectF rectF = new RectF(0.0f, 0.0f, m9289getFirstimpl, m9290getSecondimpl);
        Path path = new Path();
    ...
    private final long m9281calculateOutputSizenmZnaPc(Bitmap bitmap, Size size) {
        if (SizeKt.isOriginal(size)) {
            return IntPair.m9285constructorimpl(bitmap.getWidth(), bitmap.getHeight());
        }
        ...
        if ((component1 instanceof Dimension.Pixels) && (component2 instanceof Dimension.Pixels)) {
            return IntPair.m9285constructorimpl(((Dimension.Pixels) component1).m9280unboximpl(), ((Dimension.Pixels) component2).m9280unboximpl());
        }
        int width = bitmap.getWidth();
        int height = bitmap.getHeight();
        Dimension width2 = size.getWidth();
        int m9280unboximpl = width2 instanceof Dimension.Pixels ? ((Dimension.Pixels) width2).m9280unboximpl() : Integer.MIN_VALUE;
        Dimension height2 = size.getHeight();
        double computeSizeMultiplier = DecodeUtils.computeSizeMultiplier(width, height, m9280unboximpl, height2 instanceof Dimension.Pixels ? ((Dimension.Pixels) height2).m9280unboximpl() : Integer.MIN_VALUE, Scale.FILL);
        return IntPair.m9285constructorimpl(MathKt.roundToInt(bitmap.getWidth() * computeSizeMultiplier), MathKt.roundToInt(computeSizeMultiplier * bitmap.getHeight()));
    }
}
