        ...
        final String decodeUtf8Default(ByteBuffer buffer, int index, int size) throws InvalidProtocolBufferException {
            ...
            while (index < i) {
                ...
                if (DecodeUtil.isOneByte(b2)) {
                    ...
                    int i8 = index + 3;
                    index += 4;
                    DecodeUtil.handleFourBytes(b2, buffer.get(i4), buffer.get(index + 2), buffer.get(i8), cArr, i3);
                    i3 += 2;
                }
            }
        ...
        String decodeUtf8(byte[] bytes, int index, int size) throws InvalidProtocolBufferException {
            ...
            while (index < i) {
                ...
                if (DecodeUtil.isOneByte(b2)) {
                    ...
                    int i8 = index + 3;
                    index += 4;
                    DecodeUtil.handleFourBytes(b2, bytes[i4], bytes[index + 2], bytes[i8], cArr, i3);
                    i3 += 2;
                }
            }
            return new String(cArr, 0, i3);
        ...
        int encodeUtf8(String in, byte[] out, int offset, int length) {
            ...
            while (i4 < length2) {
                ...
                if (charAt2 < 128 && i5 < i3) {
                    ...
                    } else if (55296 <= charAt2 && charAt2 <= 57343 && ((i = i4 + 1) == in.length() || !Character.isSurrogatePair(charAt2, in.charAt(i)))) {
        ...
        String decodeUtf8Direct(ByteBuffer buffer, int index, int size) throws InvalidProtocolBufferException {
            ...
            while (addressOffset < j) {
                ...
                if (DecodeUtil.isOneByte(b2)) {
                    ...
                    addressOffset += 4;
                    DecodeUtil.handleFourBytes(b2, UnsafeUtil.getByte(j2), UnsafeUtil.getByte(2 + addressOffset), UnsafeUtil.getByte(j5), cArr, i2);
                    i2 += 2;
                }
            }
