...
public final class TrieNode<K, V> {
    ...
    private final TrieNode<K, V> mutableCollisionPutAll(TrieNode<K, V> trieNode, DeltaCounter deltaCounter, MutabilityOwnership mutabilityOwnership) {
        CommonFunctionsKt.m3633assert(this.nodeMap == 0);
        CommonFunctionsKt.m3633assert(this.dataMap == 0);
        CommonFunctionsKt.m3633assert(trieNode.nodeMap == 0);
        CommonFunctionsKt.m3633assert(trieNode.dataMap == 0);
        Object[] objArr = this.buffer;
    ...
    public final TrieNode<K, V> mutablePutAll(TrieNode<K, V> trieNode, int i, DeltaCounter deltaCounter, PersistentHashMapBuilder<K, V> persistentHashMapBuilder) {
        ...
        while (i6 != 0) {
            ...
            if (!trieNode.hasEntryAt$runtime_release(lowestOneBit3)) {
                ...
                trieNode2.buffer[i11] = keyAtIndex(entryKeyIndex$runtime_release);
                trieNode2.buffer[i11 + 1] = valueAtKeyIndex(entryKeyIndex$runtime_release);
            } else {
                int entryKeyIndex$runtime_release2 = trieNode.entryKeyIndex$runtime_release(lowestOneBit3);
                trieNode2.buffer[i11] = trieNode.keyAtIndex(entryKeyIndex$runtime_release2);
                trieNode2.buffer[i11 + 1] = trieNode.valueAtKeyIndex(entryKeyIndex$runtime_release2);
    ...
    public final TrieNode<K, V> mutablePut(int i, K k, V v, int i2, PersistentHashMapBuilder<K, V> persistentHashMapBuilder) {
        ...
        if (hasEntryAt$runtime_release(indexSegment)) {
            ...
            if (i2 == 30) {
                ...
                persistentHashMapBuilder2 = persistentHashMapBuilder;
                mutablePut = nodeAtIndex$runtime_release.mutablePut(i, k, v, i2 + 5, persistentHashMapBuilder2);
            }
    ...
    public final TrieNode<K, V> remove(int i, K k, V v, int i2) {
        ...
        if (hasEntryAt$runtime_release(indexSegment)) {
            int entryKeyIndex$runtime_release = entryKeyIndex$runtime_release(indexSegment);
            if (Intrinsics.areEqual(k, keyAtIndex(entryKeyIndex$runtime_release)) && Intrinsics.areEqual(v, valueAtKeyIndex(entryKeyIndex$runtime_release))) {
                return removeEntryAtIndex(entryKeyIndex$runtime_release, indexSegment);
            }
        } else if (hasNodeAt(indexSegment)) {
            int nodeIndex$runtime_release = nodeIndex$runtime_release(indexSegment);
