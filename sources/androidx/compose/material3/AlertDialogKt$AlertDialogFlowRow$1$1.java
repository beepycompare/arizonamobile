...
final class AlertDialogKt$AlertDialogFlowRow$1$1 implements MeasurePolicy {
    ...
    public final MeasureResult mo53measure3p2s80s(final MeasureScope measureScope, List<? extends Measurable> list, long j) {
        ...
        while (i2 < size) {
            int i3 = size;
            Placeable mo6698measureBRTryo0 = list2.get(i2).mo6698measureBRTryo0(j);
            Ref.IntRef intRef5 = intRef3;
            intRef3 = intRef5;
            if (measure_3p2s80s$canAddToCurrentSequence(arrayList4, intRef5, measureScope, f, j, mo6698measureBRTryo0)) {
                i = i2;
            } else {
                i = i2;
            ...
            if (!arrayList4.isEmpty()) {
                intRef3.element += measureScope.mo424roundToPx0680j_4(f);
            }
            arrayList4.add(mo6698measureBRTryo0);
            intRef3.element += mo6698measureBRTryo0.getWidth();
            intRef4.element = Math.max(intRef4.element, mo6698measureBRTryo0.getHeight());
            i2 = i + 1;
            list2 = list;
            size = i3;
        }
        ...
        final int max = Math.max(intRef.element, Constraints.m7951getMinWidthimpl(j));
        int max2 = Math.max(intRef2.element, Constraints.m7950getMinHeightimpl(j));
    ...
    private static final boolean measure_3p2s80s$canAddToCurrentSequence(List<Placeable> list, Ref.IntRef intRef, MeasureScope measureScope, float f, long j, Placeable placeable) {
        return list.isEmpty() || (intRef.element + measureScope.mo424roundToPx0680j_4(f)) + placeable.getWidth() <= Constraints.m7949getMaxWidthimpl(j);
    }
    ...
    private static final void measure_3p2s80s$startNewSequence(List<List<Placeable>> list, Ref.IntRef intRef, MeasureScope measureScope, float f, List<Placeable> list2, List<Integer> list3, Ref.IntRef intRef2, List<Integer> list4, Ref.IntRef intRef3, Ref.IntRef intRef4) {
        if (!list.isEmpty()) {
            intRef.element += measureScope.mo424roundToPx0680j_4(f);
        }
    ...
    public static final Unit measure_3p2s80s$lambda$4(List list, MeasureScope measureScope, float f, int i, List list2, Placeable.PlacementScope placementScope) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            ...
            int i3 = 0;
            while (i3 < size2) {
                iArr[i3] = ((Placeable) list3.get(i3)).getWidth() + (i3 < CollectionsKt.getLastIndex(list3) ? measureScope.mo424roundToPx0680j_4(f) : 0);
                i3++;
            }
