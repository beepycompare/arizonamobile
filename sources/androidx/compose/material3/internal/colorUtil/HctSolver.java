...
public final class HctSolver {
    ...
    private final double intercept(double d, double d2, double d3) {
        return d3 == d ? d3 : (d2 - d) / (d3 - d);
    }

    private final boolean isBounded(double d) {
        return FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE <= d && d <= 100.0d;
    }

    private final double sanitizeDegreesDouble(double d) {
        double d2 = d % 360.0d;
        return d2 < FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE ? d2 + 360.0d : d2;
    }

    private final double sanitizeRadians(double d) {
    ...
    private final double chromaticAdaptation(double d) {
        ...
        return ((CamUtils.INSTANCE.signum(d) * 400.0d) * pow) / (pow + 27.13d);
    }

    private final double hueOf(double[] dArr) {
        ...
        double chromaticAdaptation = chromaticAdaptation(d4);
        double chromaticAdaptation2 = chromaticAdaptation(d5);
        double chromaticAdaptation3 = chromaticAdaptation((d * dArr5[0]) + (d2 * dArr5[1]) + (d3 * dArr5[2]));
    ...
    private final boolean areInCyclicOrder(double d, double d2, double d3) {
        return sanitizeRadians(d2 - d) < sanitizeRadians(d3 - d);
    }

    private final double[] lerpPoint(double[] dArr, double d, double[] dArr2) {
        double d2 = dArr[0];
        double d3 = dArr[1];
        double d4 = dArr[2];
    ...
    private final double[] setCoordinate(double[] dArr, double d, double[] dArr2, int i) {
        return lerpPoint(dArr, intercept(dArr[i], d, dArr2[i]), dArr2);
    }

    private final double[] nthVertex(double d, int i) {
    ...
    */
    private final double[][] bisectToSegment(double d, double d2) {
        double d3;
        ...
        while (i < 12) {
            char c3 = c;
            double[] nthVertex = hctSolver.nthVertex(d, i);
            if (nthVertex[c3] < FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE) {
                d3 = d5;
            } else {
    ...
    private final int criticalPlaneBelow(double d) {
        return (int) Math.floor(d - 0.5d);
    }

    private final int criticalPlaneAbove(double d) {
        return (int) Math.ceil(d - 0.5d);
    }

    private final int bisectToLimit(double d, double d2) {
        ...
        while (i2 < 3) {
            ...
            if (d3 == d4) {
                ...
                if (d3 < d4) {
                    criticalPlaneAbove = hctSolver.criticalPlaneBelow(hctSolver.trueDelinearized(d3));
                    criticalPlaneBelow = hctSolver.criticalPlaneAbove(hctSolver.trueDelinearized(dArr2[i2]));
                } else {
                    criticalPlaneAbove = hctSolver.criticalPlaneAbove(hctSolver.trueDelinearized(d3));
    ...
    private final double inverseChromaticAdaptation(double d) {
        ...
        return CamUtils.INSTANCE.signum(d) * Math.pow(Math.max((double) FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE, (27.13d * abs) / (400.0d - abs)), 2.380952380952381d);
    }
    ...
    public final int solveToInt(double d, double d2, double d3) {
        if (d2 < 1.0E-4d || d3 < 1.0E-4d || d3 > 99.9999d) {
            return CamUtils.INSTANCE.argbFromLstar(d3);
        }
        double radians = Math.toRadians(sanitizeDegreesDouble(d));
        double yFromLstar = CamUtils.INSTANCE.yFromLstar(d3);
