...
public abstract class RippleIndicationInstance implements IndicationInstance {
    ...
    public final void m2131drawStateLayerH2RKhps(DrawScope drawScope, float f, long j) {
        float f2;
        StateLayer stateLayer = this.stateLayer;
        if (Float.isNaN(f)) {
            f2 = RippleAnimationKt.m2127getRippleEndRadiuscSwnlzA(drawScope, this.bounded, drawScope.mo5922getSizeNHjbRc());
        } else {
            f2 = drawScope.mo434toPx0680j_4(f);
        }
        stateLayer.m2139drawStateLayermxwnekA(drawScope, f2, j);
    }
}
