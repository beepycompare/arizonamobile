...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long mo691calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j) {
        int maxIntrinsicHeight;
        if (this.height == IntrinsicSize.Min) {
            maxIntrinsicHeight = measurable.minIntrinsicHeight(Constraints.m6637getMaxWidthimpl(j));
        } else {
            maxIntrinsicHeight = measurable.maxIntrinsicHeight(Constraints.m6637getMaxWidthimpl(j));
        }
        if (maxIntrinsicHeight < 0) {
            maxIntrinsicHeight = 0;
        }
        return Constraints.Companion.m6648fixedHeightOenEA2s(maxIntrinsicHeight);
    }
