...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long mo1003calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j) {
        int maxIntrinsicHeight;
        if (this.height == IntrinsicSize.Min) {
            maxIntrinsicHeight = measurable.minIntrinsicHeight(Constraints.m8113getMaxWidthimpl(j));
        } else {
            maxIntrinsicHeight = measurable.maxIntrinsicHeight(Constraints.m8113getMaxWidthimpl(j));
        }
        if (maxIntrinsicHeight < 0) {
            maxIntrinsicHeight = 0;
        }
        return Constraints.Companion.m8124fixedHeightOenEA2s(maxIntrinsicHeight);
    }
