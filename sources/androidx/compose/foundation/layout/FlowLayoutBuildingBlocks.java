...
public final class FlowLayoutBuildingBlocks {
    ...
    public static final class WrapEllipsisInfo {
        ...
        public final long m898getEllipsisSizeOO21N7I() {
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean z, int i, int i2, int i3, int i4) {
        ...
        if (wrapInfo.isLastItemInContainer() && (ellipsisInfo$foundation_layout = this.overflow.ellipsisInfo$foundation_layout(z, i, i2)) != null) {
            ellipsisInfo$foundation_layout.setPlaceEllipsisOnLastContentLine(i >= 0 && (i4 == 0 || (i3 - IntIntPair.m28getFirstimpl(ellipsisInfo$foundation_layout.m898getEllipsisSizeOO21N7I()) >= 0 && i4 < this.maxItemsInMainAxis)));
    ...
    public final WrapInfo m897getWrapInfoOpUlnko(boolean z, int i, long j, IntIntPair intIntPair, int i2, int i3, int i4, boolean z2, boolean z3) {
        int i5 = i3 + i4;
        if (intIntPair == null) {
        ...
        if (i != 0 && (i >= this.maxItemsInMainAxis || IntIntPair.m28getFirstimpl(j) - IntIntPair.m28getFirstimpl(intIntPair.m32unboximpl()) < 0)) {
            ...
            return new WrapInfo(true, m897getWrapInfoOpUlnko(z, 0, IntIntPair.m24constructorimpl(Constraints.m8113getMaxWidthimpl(this.constraints), (IntIntPair.m29getSecondimpl(j) - this.crossAxisSpacing) - i4), IntIntPair.m21boximpl(IntIntPair.m24constructorimpl(IntIntPair.m28getFirstimpl(intIntPair.m32unboximpl()) - this.mainAxisSpacing, IntIntPair.m29getSecondimpl(intIntPair.m32unboximpl()))), i2 + 1, i5, 0, true, false).isLastItemInContainer());
        }
        int max = i3 + Math.max(i4, IntIntPair.m29getSecondimpl(intIntPair.m32unboximpl()));
        IntIntPair m905ellipsisSizeF35zmw$foundation_layout = z3 ? null : this.overflow.m905ellipsisSizeF35zmw$foundation_layout(z, i2, max);
        if (m905ellipsisSizeF35zmw$foundation_layout != null) {
            m905ellipsisSizeF35zmw$foundation_layout.m32unboximpl();
            if (i + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m28getFirstimpl(j) - IntIntPair.m28getFirstimpl(intIntPair.m32unboximpl())) - this.mainAxisSpacing) - IntIntPair.m28getFirstimpl(m905ellipsisSizeF35zmw$foundation_layout.m32unboximpl()) < 0) {
                if (z3) {
                    return new WrapInfo(true, true);
                }
                WrapInfo m897getWrapInfoOpUlnko = m897getWrapInfoOpUlnko(false, 0, IntIntPair.m24constructorimpl(Constraints.m8113getMaxWidthimpl(this.constraints), (IntIntPair.m29getSecondimpl(j) - this.crossAxisSpacing) - Math.max(i4, IntIntPair.m29getSecondimpl(intIntPair.m32unboximpl()))), m905ellipsisSizeF35zmw$foundation_layout, i2 + 1, max, 0, true, true);
                return new WrapInfo(m897getWrapInfoOpUlnko.isLastItemInContainer(), m897getWrapInfoOpUlnko.isLastItemInContainer());
            }
        }
