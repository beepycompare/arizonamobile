...
public final class DecodeHelper {
        ...
        while (this.parcel.dataAvail() > 1) {
            ...
            if (decodeByte != 1) {
                if (decodeByte != 2) {
                    if (decodeByte != 3) {
                        if (decodeByte != 4) {
                            if (decodeByte != 5) {
                                        ...
                                        if (decodeByte != 8) {
                                            if (decodeByte != 9) {
                                                if (decodeByte != 10) {
                                                    ...
                                                    mutableSpanStyle.m580setBackground8_81llA(m563decodeColor0d7_KjU());
                                                }
                                            } else if (dataAvailable() < 8) {
                                                break;
                                            } else {
                                            ...
                                            mutableSpanStyle.m581setBaselineShift_isdbwI(BaselineShift.m6440boximpl(m561decodeBaselineShifty9eOQZs()));
                                        }
                                    } else if (dataAvailable() < 5) {
                                        break;
                                    } else {
                                        mutableSpanStyle.m586setLetterSpacingR2X_6o(m566decodeTextUnitXSAIIZE());
                                    }
                                } else {
                                ...
                                mutableSpanStyle.m585setFontSynthesistDdu0R4(FontSynthesis.m6267boximpl(m565decodeFontSynthesisGVVA2EU()));
                            }
                        } else if (dataAvailable() < 1) {
                            break;
                        } else {
                            mutableSpanStyle.m584setFontStylemLjRB2g(FontStyle.m6256boximpl(m564decodeFontStyle_LCdwA()));
                        }
                    } else if (dataAvailable() < 4) {
                        break;
                    } else {
                    ...
                    mutableSpanStyle.m583setFontSizeR2X_6o(m566decodeTextUnitXSAIIZE());
                }
            } else if (dataAvailable() < 8) {
                break;
            } else {
                mutableSpanStyle.m582setColor8_81llA(m563decodeColor0d7_KjU());
            }
        }
    ...
    public final long m563decodeColor0d7_KjU() {
        return Color.m4074constructorimpl(m562decodeULongsVKNKU());
    }

    /* renamed from: decodeTextUnit-XSAIIZE  reason: not valid java name */
    public final long m566decodeTextUnitXSAIIZE() {
    ...
    public final int m564decodeFontStyle_LCdwA() {
        byte decodeByte = decodeByte();
        if (decodeByte == 0) {
    ...
    public final int m565decodeFontSynthesisGVVA2EU() {
        byte decodeByte = decodeByte();
        if (decodeByte == 0) {
    ...
    private final float m561decodeBaselineShifty9eOQZs() {
    ...
    private final Shadow decodeShadow() {
        long m563decodeColor0d7_KjU = m563decodeColor0d7_KjU();
        float decodeFloat = decodeFloat();
        float decodeFloat2 = decodeFloat();
        return new Shadow(m563decodeColor0d7_KjU, Offset.m3829constructorimpl((Float.floatToRawIntBits(decodeFloat2) & 4294967295L) | (Float.floatToRawIntBits(decodeFloat) << 32)), decodeFloat(), null);
    }
    ...
    private final long m562decodeULongsVKNKU() {
        return ULong.m8619constructorimpl(this.parcel.readLong());
    }
