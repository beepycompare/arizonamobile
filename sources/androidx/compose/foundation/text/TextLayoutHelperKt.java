...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m1675canReuse7_7YC6M(TextLayoutResult textLayoutResult, AnnotatedString annotatedString, TextStyle textStyle, List<AnnotatedString.Range<Placeholder>> list, int i, boolean z, int i2, Density density, LayoutDirection layoutDirection, FontFamily.Resolver resolver, long j) {
        TextLayoutInput layoutInput = textLayoutResult.getLayoutInput();
        if (!textLayoutResult.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() && Intrinsics.areEqual(layoutInput.getText(), annotatedString) && layoutInput.getStyle().hasSameLayoutAffectingAttributes(textStyle) && Intrinsics.areEqual(layoutInput.getPlaceholders(), list) && layoutInput.getMaxLines() == i && layoutInput.getSoftWrap() == z && TextOverflow.m8086equalsimpl0(layoutInput.m7575getOverflowgIe3tQ8(), i2) && Intrinsics.areEqual(layoutInput.getDensity(), density) && layoutInput.getLayoutDirection() == layoutDirection && Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), resolver) && Constraints.m8115getMinWidthimpl(j) == Constraints.m8115getMinWidthimpl(layoutInput.m7574getConstraintsmsEJaDk())) {
            if (z || TextOverflow.m8086equalsimpl0(i2, TextOverflow.Companion.m8096getEllipsisgIe3tQ8())) {
                return Constraints.m8113getMaxWidthimpl(j) == Constraints.m8113getMaxWidthimpl(layoutInput.m7574getConstraintsmsEJaDk()) && Constraints.m8112getMaxHeightimpl(j) == Constraints.m8112getMaxHeightimpl(layoutInput.m7574getConstraintsmsEJaDk());
            }
            return true;
        }
        return false;
    ...
    public static final boolean m1676isPositionInsideSelectionuaM50fQ(TextLayoutResult textLayoutResult, long j, TextRange textRange) {
        if (textRange == null || TextRange.m7602getCollapsedimpl(textRange.m7612unboximpl())) {
            return false;
        }
        int m7578getOffsetForPositionk4lQ0M = textLayoutResult.m7578getOffsetForPositionk4lQ0M(j);
        return isPositionInsideSelection_uaM50fQ$isOffsetSelectedAndContainsPosition(textRange, textLayoutResult, j, m7578getOffsetForPositionk4lQ0M) || isPositionInsideSelection_uaM50fQ$isOffsetSelectedAndContainsPosition(textRange, textLayoutResult, j, m7578getOffsetForPositionk4lQ0M - 1);
    }
    ...
    private static final boolean isPositionInsideSelection_uaM50fQ$isOffsetSelectedAndContainsPosition(TextRange textRange, TextLayoutResult textLayoutResult, long j, int i) {
        return TextRange.m7599containsimpl(textRange.m7612unboximpl(), i) && textLayoutResult.getBoundingBox(i).m5129containsk4lQ0M(j);
    }
