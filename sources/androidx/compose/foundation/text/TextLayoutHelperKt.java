...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m1384canReuse7_7YC6M(TextLayoutResult textLayoutResult, AnnotatedString annotatedString, TextStyle textStyle, List<AnnotatedString.Range<Placeholder>> list, int i, boolean z, int i2, Density density, LayoutDirection layoutDirection, FontFamily.Resolver resolver, long j) {
        TextLayoutInput layoutInput = textLayoutResult.getLayoutInput();
        if (!textLayoutResult.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() && Intrinsics.areEqual(layoutInput.getText(), annotatedString) && layoutInput.getStyle().hasSameLayoutAffectingAttributes(textStyle) && Intrinsics.areEqual(layoutInput.getPlaceholders(), list) && layoutInput.getMaxLines() == i && layoutInput.getSoftWrap() == z && TextOverflow.m7922equalsimpl0(layoutInput.m7425getOverflowgIe3tQ8(), i2) && Intrinsics.areEqual(layoutInput.getDensity(), density) && layoutInput.getLayoutDirection() == layoutDirection && Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), resolver) && Constraints.m7951getMinWidthimpl(j) == Constraints.m7951getMinWidthimpl(layoutInput.m7424getConstraintsmsEJaDk())) {
            if (z || TextOverflow.m7922equalsimpl0(i2, TextOverflow.Companion.m7932getEllipsisgIe3tQ8())) {
                return Constraints.m7949getMaxWidthimpl(j) == Constraints.m7949getMaxWidthimpl(layoutInput.m7424getConstraintsmsEJaDk()) && Constraints.m7948getMaxHeightimpl(j) == Constraints.m7948getMaxHeightimpl(layoutInput.m7424getConstraintsmsEJaDk());
            }
            return true;
        }
        return false;
    ...
    public static final boolean m1385isPositionInsideSelectionuaM50fQ(TextLayoutResult textLayoutResult, long j, TextRange textRange) {
        if (textRange == null || TextRange.m7452getCollapsedimpl(textRange.m7462unboximpl())) {
            return false;
        }
        int m7428getOffsetForPositionk4lQ0M = textLayoutResult.m7428getOffsetForPositionk4lQ0M(j);
        return isPositionInsideSelection_uaM50fQ$isOffsetSelectedAndContainsPosition(textRange, textLayoutResult, j, m7428getOffsetForPositionk4lQ0M) || isPositionInsideSelection_uaM50fQ$isOffsetSelectedAndContainsPosition(textRange, textLayoutResult, j, m7428getOffsetForPositionk4lQ0M - 1);
    }
    ...
    private static final boolean isPositionInsideSelection_uaM50fQ$isOffsetSelectedAndContainsPosition(TextRange textRange, TextLayoutResult textLayoutResult, long j, int i) {
        return TextRange.m7449containsimpl(textRange.m7462unboximpl(), i) && textLayoutResult.getBoundingBox(i).m5062containsk4lQ0M(j);
    }
