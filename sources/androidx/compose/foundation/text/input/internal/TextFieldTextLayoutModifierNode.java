...
public final class TextFieldTextLayoutModifierNode extends DelegatingNode implements LayoutModifierNode, GlobalPositionAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    public MeasureResult mo69measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        TextLayoutResult m1883layoutWithNewMeasureInputshBUhpc = this.textLayoutState.m1883layoutWithNewMeasureInputshBUhpc(measureScope, measureScope.getLayoutDirection(), (FontFamily.Resolver) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, CompositionLocalsKt.getLocalFontFamilyResolver()), j);
        final Placeable mo6818measureBRTryo0 = measurable.mo6818measureBRTryo0(Constraints.Companion.m8122fitPrioritizingWidthZbe2FdA((int) (m1883layoutWithNewMeasureInputshBUhpc.m7579getSizeYbymL2g() >> 32), (int) (m1883layoutWithNewMeasureInputshBUhpc.m7579getSizeYbymL2g() >> 32), (int) (m1883layoutWithNewMeasureInputshBUhpc.m7579getSizeYbymL2g() & 4294967295L), (int) (m1883layoutWithNewMeasureInputshBUhpc.m7579getSizeYbymL2g() & 4294967295L)));
        TextLayoutState textLayoutState = this.textLayoutState;
        if (this.singleLine) {
            m8160constructorimpl = measureScope.mo431toDpu2uoSUM(TextDelegateKt.ceilToIntPx(m1883layoutWithNewMeasureInputshBUhpc.getLineBottom(0)));
        } else {
            m8160constructorimpl = Dp.m8160constructorimpl(0.0f);
        }
        textLayoutState.m1884setMinHeightForSingleLineField0680j_4(m8160constructorimpl);
        LinkedHashMap linkedHashMap = this.baselineCache;
        if (linkedHashMap == null) {
            linkedHashMap = new LinkedHashMap(2);
        }
        linkedHashMap.put(AlignmentLineKt.getFirstBaseline(), Integer.valueOf(Math.round(m1883layoutWithNewMeasureInputshBUhpc.getFirstBaseline())));
        linkedHashMap.put(AlignmentLineKt.getLastBaseline(), Integer.valueOf(Math.round(m1883layoutWithNewMeasureInputshBUhpc.getLastBaseline())));
        this.baselineCache = linkedHashMap;
        Map<AlignmentLine, Integer> map = this.baselineCache;
        Intrinsics.checkNotNull(map);
        return measureScope.layout((int) (m1883layoutWithNewMeasureInputshBUhpc.m7579getSizeYbymL2g() >> 32), (int) (m1883layoutWithNewMeasureInputshBUhpc.m7579getSizeYbymL2g() & 4294967295L), map, new Function1() { // from class: androidx.compose.foundation.text.input.internal.TextFieldTextLayoutModifierNode$$ExternalSyntheticLambda0
