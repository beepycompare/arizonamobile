...
public final class AnchoredDraggableState<T> {
    ...
    private final T calculateTargetValue(float f) {
        if (ComposeFoundationFlags.isAnchoredDraggableTargetValueCalculationFixEnabled) {
            return calculateTargetValueWithFix(f);
        }
        if (!Float.isNaN(f)) {
            T closestAnchor = getAnchors().closestAnchor(f);
            return closestAnchor == null ? getCurrentValue() : closestAnchor;
        }
        return getCurrentValue();
    }

    private final T calculateTargetValueWithFix(float f) {
        if (!Float.isNaN(f)) {
            float positionOf = getAnchors().positionOf(getCurrentValue());
            if (Float.isNaN(positionOf) || f == positionOf) {
                return getCurrentValue();
            }
            T closestAnchor = getAnchors().closestAnchor(f);
            return closestAnchor == null ? getCurrentValue() : closestAnchor;
