...
public final class BringIntoViewRequestPriorityQueue {
    ...
    public final boolean enqueue(final ContentInViewNode.Request request) {
        ...
        if (invoke == null) {
            ...
            request.getContinuation().resumeWith(Result.m9916constructorimpl(Unit.INSTANCE));
            return false;
        }
    ...
    public final void resumeAndRemoveAll() {
        ...
        if (first <= last) {
            while (true) {
                ...
                Result.Companion companion = Result.Companion;
                this.requests.content[first].getContinuation().resumeWith(Result.m9916constructorimpl(unit));
                if (first == last) {
                    break;
                }
                first++;
    ...
    public final void resumeAndRemoveWhile(Function1<? super Rect, Boolean> function1) {
        while (this.requests.getSize() != 0 && function1.invoke(((ContentInViewNode.Request) this.requests.last()).getCurrentBounds().invoke()).booleanValue()) {
            Unit unit = Unit.INSTANCE;
            Result.Companion companion = Result.Companion;
            ((ContentInViewNode.Request) this.requests.removeAt(this.requests.getSize() - 1)).getContinuation().resumeWith(Result.m9916constructorimpl(unit));
        }
    }
