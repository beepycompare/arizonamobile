...
final class LazyGridCacheWindowScope implements CacheWindowScope {
    ...
    public int getMainAxisExtraSpaceStart() {
        return Math.abs(RangesKt.coerceAtMost(LazyGridSnapLayoutInfoProviderKt.offsetOnMainAxis((LazyGridItemInfo) CollectionsKt.first((List<? extends Object>) getLayoutInfo().getVisibleItemsInfo()), getLayoutInfo().getOrientation()) + getLayoutInfo().getBeforeContentPadding(), 0));
    }

    @Override // androidx.compose.foundation.lazy.layout.CacheWindowScope
    public int getMainAxisExtraSpaceEnd() {
        LazyGridItemInfo lazyGridItemInfo = (LazyGridItemInfo) CollectionsKt.last((List<? extends Object>) getLayoutInfo().getVisibleItemsInfo());
    ...
    public int getFirstVisibleLineIndex() {
        return getLineIndex((LazyGridItemInfo) CollectionsKt.first((List<? extends Object>) getLayoutInfo().getVisibleItemsInfo()));
    }

    @Override // androidx.compose.foundation.lazy.layout.CacheWindowScope
    public int getLastVisibleLineIndex() {
        return getLineIndex((LazyGridItemInfo) CollectionsKt.last((List<? extends Object>) getLayoutInfo().getVisibleItemsInfo()));
    }
    ...
    @Override // androidx.compose.foundation.lazy.layout.CacheWindowScope
    public List<LazyLayoutPrefetchState.PrefetchHandle> schedulePrefetch(final int i, final Function2<? super Integer, ? super Integer, Unit> function2) {
    ...
    public int getVisibleItemSize(int i) {
        int firstVisibleLineIndex = i + getFirstVisibleLineIndex();
        ...
        for (int i4 = 0; i4 < size2; i4++) {
            i3 = Math.max(LazyGridSnapLayoutInfoProviderKt.sizeOnMainAxis((LazyGridItemInfo) arrayList2.get(i4), getLayoutInfo().getOrientation()), i3);
        }
        return i3;
    }
    ...
    public Object getVisibleLineKey(int i) {
        ...
        LazyGridItemInfo lazyGridItemInfo2 = (LazyGridItemInfo) CollectionsKt.firstOrNull((List<? extends Object>) arrayList);
        return (lazyGridItemInfo2 == null || (key = lazyGridItemInfo2.getKey()) == null) ? CachedItem.NoKey : key;
    }

    @Override // androidx.compose.foundation.lazy.layout.CacheWindowScope
    public int getVisibleItemLine(int i) {
        return getFirstVisibleLineIndex() + i;
    }
    ...
    public int getLastIndexInLine(int i) {
        ...
        List<Pair<Integer, Constraints>> invoke = lazyGridMeasureResult.getPrefetchInfoRetriever().invoke(Integer.valueOf(i));
        if (invoke.isEmpty()) {
            return -1;
        }
        return (((Number) ((Pair) CollectionsKt.first((List<? extends Object>) invoke)).getFirst()).intValue() + invoke.size()) - 1;
    }
