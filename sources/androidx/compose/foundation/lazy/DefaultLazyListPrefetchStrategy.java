...
final class DefaultLazyListPrefetchStrategy implements LazyListPrefetchStrategy {
    ...
    public void onScroll(LazyListPrefetchScope lazyListPrefetchScope, float f, LazyListLayoutInfo lazyListLayoutInfo) {
        ...
        if (!lazyListLayoutInfo.getVisibleItemsInfo().isEmpty()) {
            ...
            if (calculateIndexToPrefetch >= 0 && calculateIndexToPrefetch < lazyListLayoutInfo.getTotalItemsCount()) {
                ...
                if (z) {
                    LazyListItemInfo lazyListItemInfo = (LazyListItemInfo) CollectionsKt.last((List<? extends Object>) lazyListLayoutInfo.getVisibleItemsInfo());
                ...
                } else if (lazyListLayoutInfo.getViewportStartOffset() - ((LazyListItemInfo) CollectionsKt.first((List<? extends Object>) lazyListLayoutInfo.getVisibleItemsInfo())).getOffset() < f && (prefetchHandle = this.currentPrefetchHandle) != null) {
    ...
    private final int calculateIndexToPrefetch(LazyListLayoutInfo lazyListLayoutInfo, boolean z) {
        if (z) {
            return ((LazyListItemInfo) CollectionsKt.last((List<? extends Object>) lazyListLayoutInfo.getVisibleItemsInfo())).getIndex() + 1;
        }
        return ((LazyListItemInfo) CollectionsKt.first((List<? extends Object>) lazyListLayoutInfo.getVisibleItemsInfo())).getIndex() - 1;
    }
