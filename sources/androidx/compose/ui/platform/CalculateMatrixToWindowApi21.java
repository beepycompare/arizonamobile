...
public final class CalculateMatrixToWindowApi21 implements CalculateMatrixToWindow {
    ...
    public void mo7279calculateMatrixToWindowEL8BTi8(View view, float[] fArr) {
        Matrix.m5601resetimpl(fArr);
        m7282transformMatrixToWindowEL8BTi8(view, fArr);
    }

    /* renamed from: transformMatrixToWindow-EL8BTi8  reason: not valid java name */
    private final void m7282transformMatrixToWindowEL8BTi8(View view, float[] fArr) {
        ViewParent parent = view.getParent();
        if (parent instanceof View) {
            m7282transformMatrixToWindowEL8BTi8((View) parent, fArr);
            m7281preTranslate3XD1CNM(fArr, -view.getScrollX(), -view.getScrollY());
            m7281preTranslate3XD1CNM(fArr, view.getLeft(), view.getTop());
        } else {
            int[] iArr = this.tmpLocation;
            view.getLocationInWindow(iArr);
            m7281preTranslate3XD1CNM(fArr, -view.getScrollX(), -view.getScrollY());
            m7281preTranslate3XD1CNM(fArr, iArr[0], iArr[1]);
        }
        android.graphics.Matrix matrix = view.getMatrix();
        if (matrix.isIdentity()) {
            return;
        }
        m7280preConcattUYjHk(fArr, matrix);
    }

    /* renamed from: preConcat-tU-YjHk  reason: not valid java name */
    private final void m7280preConcattUYjHk(float[] fArr, android.graphics.Matrix matrix) {
        AndroidMatrixConversions_androidKt.m5216setFromtUYjHk(this.tmpMatrix, matrix);
        AndroidComposeView_androidKt.m7267preTransformJiSxe2E(fArr, this.tmpMatrix);
    }

    /* renamed from: preTranslate-3XD1CNM  reason: not valid java name */
    private final void m7281preTranslate3XD1CNM(float[] fArr, float f, float f2) {
        AndroidComposeView_androidKt.m7268preTranslatecG2Xzmc(fArr, f, f2, this.tmpMatrix);
    }
}
