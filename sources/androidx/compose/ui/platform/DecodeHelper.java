...
public final class DecodeHelper {
        ...
        while (this.parcel.dataAvail() > 1) {
            ...
            if (decodeByte != 1) {
                if (decodeByte != 2) {
                    if (decodeByte != 3) {
                        if (decodeByte != 4) {
                            if (decodeByte != 5) {
                                        ...
                                        if (decodeByte != 8) {
                                            if (decodeByte != 9) {
                                                if (decodeByte != 10) {
                                                    ...
                                                    mutableSpanStyle.m7397setBackground8_81llA(m7348decodeColor0d7_KjU());
                                                }
                                            } else if (dataAvailable() < 8) {
                                                break;
                                            } else {
                                            ...
                                            mutableSpanStyle.m7398setBaselineShift_isdbwI(BaselineShift.m7998boximpl(m7346decodeBaselineShifty9eOQZs()));
                                        }
                                    } else if (dataAvailable() < 5) {
                                        break;
                                    } else {
                                        mutableSpanStyle.m7403setLetterSpacingR2X_6o(m7351decodeTextUnitXSAIIZE());
                                    }
                                } else {
                                ...
                                mutableSpanStyle.m7402setFontSynthesistDdu0R4(FontSynthesis.m7823boximpl(m7350decodeFontSynthesisGVVA2EU()));
                            }
                        } else if (dataAvailable() < 1) {
                            break;
                        } else {
                            mutableSpanStyle.m7401setFontStylemLjRB2g(FontStyle.m7812boximpl(m7349decodeFontStyle_LCdwA()));
                        }
                    } else if (dataAvailable() < 4) {
                        break;
                    } else {
                    ...
                    mutableSpanStyle.m7400setFontSizeR2X_6o(m7351decodeTextUnitXSAIIZE());
                }
            } else if (dataAvailable() < 8) {
                break;
            } else {
                mutableSpanStyle.m7399setColor8_81llA(m7348decodeColor0d7_KjU());
            }
        }
    ...
    public final int m7349decodeFontStyle_LCdwA() {
        byte decodeByte = decodeByte();
        if (decodeByte == 0) {
    ...
    public final int m7350decodeFontSynthesisGVVA2EU() {
        byte decodeByte = decodeByte();
        if (decodeByte == 0) {
    ...
    private final float m7346decodeBaselineShifty9eOQZs() {
    ...
    private final Shadow decodeShadow() {
        long m7348decodeColor0d7_KjU = m7348decodeColor0d7_KjU();
        float decodeFloat = decodeFloat();
        float decodeFloat2 = decodeFloat();
        return new Shadow(m7348decodeColor0d7_KjU, Offset.m5171constructorimpl((Float.floatToRawIntBits(decodeFloat2) & 4294967295L) | (Float.floatToRawIntBits(decodeFloat) << 32)), decodeFloat(), null);
    }
