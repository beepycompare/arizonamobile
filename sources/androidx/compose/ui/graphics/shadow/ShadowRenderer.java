...
public abstract class ShadowRenderer {
    ...
    private long shadowTintColor = Color.Companion.m5384getUnspecified0d7_KjU();
    private long cornerRadius = CornerRadius.Companion.m5075getZerokKHJgLs();
    private long generatedSize = Size.Companion.m5180getUnspecifiedNHjbRc();
    ...
    protected abstract void mo6060buildShadow_SMYjrA(DrawScope drawScope, long j, long j2, Path path);

    /* renamed from: onDrawShadow-MLmccfk */
    protected abstract void mo6061onDrawShadowMLmccfk(DrawScope drawScope, long j, long j2, Path path, float f, ColorFilter colorFilter, Brush brush, int i);
    ...
    public final void m6071drawShadowerFMhIw(DrawScope drawScope, ColorFilter colorFilter, long j, long j2, Brush brush, float f, int i) {
        updateParamsFromOutline(this.outline);
        if (colorFilter == null) {
            colorFilter = (brush != null || j2 == 16) ? null : m6070obtainTint8_81llA(j2);
        }
        ColorFilter colorFilter2 = colorFilter;
        long j3 = this.generatedSize;
        if (j3 == InlineClassHelperKt.UnspecifiedPackedFloats || !Size.m5168equalsimpl0(j3, j) || this.generatedLayoutDirection != drawScope.getLayoutDirection() || this.generatedDensity != drawScope.getDensity()) {
            mo6060buildShadow_SMYjrA(drawScope, j, this.cornerRadius, this.path);
            this.generatedSize = j;
            this.generatedLayoutDirection = drawScope.getLayoutDirection();
            this.generatedDensity = drawScope.getDensity();
        }
        mo6061onDrawShadowMLmccfk(drawScope, j, this.cornerRadius, this.path, f, colorFilter2, brush, i);
    }

    public void invalidateShadow() {
        this.generatedSize = Size.Companion.m5180getUnspecifiedNHjbRc();
    ...
    private final ColorFilter m6070obtainTint8_81llA(long j) {
        ColorFilter colorFilter = this.shadowTint;
        if (colorFilter == null || !Color.m5349equalsimpl0(this.shadowTintColor, j)) {
            ColorFilter m5389tintxETnrds$default = ColorFilter.Companion.m5389tintxETnrds$default(ColorFilter.Companion, j, 0, 2, null);
            this.shadowTintColor = j;
            this.shadowTint = m5389tintxETnrds$default;
            return m5389tintxETnrds$default;
        }
        return colorFilter;
    }
    ...
    private final void updateParamsFromOutline(Outline outline) {
        if (outline instanceof Outline.Generic) {
            ...
            this.cornerRadius = CornerRadius.Companion.m5075getZerokKHJgLs();
        } else if (outline instanceof Outline.Rounded) {
            Outline.Rounded rounded = (Outline.Rounded) outline;
            if (RoundRectKt.isSimple(rounded.getRoundRect())) {
                this.path = null;
                this.cornerRadius = rounded.getRoundRect().m5153getTopLeftCornerRadiuskKHJgLs();
                return;
            }
