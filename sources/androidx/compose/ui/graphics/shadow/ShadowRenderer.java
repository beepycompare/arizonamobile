...
public abstract class ShadowRenderer {
    ...
    private long shadowTintColor = Color.Companion.m4808getUnspecified0d7_KjU();
    private long cornerRadius = CornerRadius.Companion.m4499getZerokKHJgLs();
    private long generatedSize = Size.Companion.m4604getUnspecifiedNHjbRc();
    ...
    protected abstract void mo5484buildShadow_SMYjrA(DrawScope drawScope, long j, long j2, Path path);

    /* renamed from: onDrawShadow-MLmccfk */
    protected abstract void mo5485onDrawShadowMLmccfk(DrawScope drawScope, long j, long j2, Path path, float f, ColorFilter colorFilter, Brush brush, int i);
    ...
    public final void m5495drawShadowerFMhIw(DrawScope drawScope, ColorFilter colorFilter, long j, long j2, Brush brush, float f, int i) {
        updateParamsFromOutline(this.outline);
        if (colorFilter == null) {
            colorFilter = (brush != null || j2 == 16) ? null : m5494obtainTint8_81llA(j2);
        }
        ColorFilter colorFilter2 = colorFilter;
        long j3 = this.generatedSize;
        if (j3 == InlineClassHelperKt.UnspecifiedPackedFloats || !Size.m4592equalsimpl0(j3, j) || this.generatedLayoutDirection != drawScope.getLayoutDirection() || this.generatedDensity != drawScope.getDensity()) {
            mo5484buildShadow_SMYjrA(drawScope, j, this.cornerRadius, this.path);
            this.generatedSize = j;
            this.generatedLayoutDirection = drawScope.getLayoutDirection();
            this.generatedDensity = drawScope.getDensity();
        }
        mo5485onDrawShadowMLmccfk(drawScope, j, this.cornerRadius, this.path, f, colorFilter2, brush, i);
    }

    public void invalidateShadow() {
        this.generatedSize = Size.Companion.m4604getUnspecifiedNHjbRc();
    ...
    private final ColorFilter m5494obtainTint8_81llA(long j) {
        ColorFilter colorFilter = this.shadowTint;
        if (colorFilter == null || !Color.m4773equalsimpl0(this.shadowTintColor, j)) {
            ColorFilter m4813tintxETnrds$default = ColorFilter.Companion.m4813tintxETnrds$default(ColorFilter.Companion, j, 0, 2, null);
            this.shadowTintColor = j;
            this.shadowTint = m4813tintxETnrds$default;
            return m4813tintxETnrds$default;
        }
        return colorFilter;
    }
    ...
    private final void updateParamsFromOutline(Outline outline) {
        if (outline instanceof Outline.Generic) {
            ...
            this.cornerRadius = CornerRadius.Companion.m4499getZerokKHJgLs();
        } else if (outline instanceof Outline.Rounded) {
            Outline.Rounded rounded = (Outline.Rounded) outline;
            if (RoundRectKt.isSimple(rounded.getRoundRect())) {
                this.path = null;
                this.cornerRadius = rounded.getRoundRect().m4577getTopLeftCornerRadiuskKHJgLs();
                return;
            }
