...
public final class MotionMeasurer extends Measurer2 {
    ...
    private final void m8433measureConstraintSethBUhpc(int i, ConstraintSet constraintSet, List<? extends Measurable> list, long j) {
        String str;
        Object layoutId;
        getState().reset();
    ...
    public final long m8437performInterpolationMeasureLzAeyeM(long j, LayoutDirection layoutDirection, ConstraintSet constraintSet, ConstraintSet constraintSet2, TransitionImpl transitionImpl, List<? extends Measurable> list, Map<Measurable, Placeable> map, int i, float f, CompositionSource compositionSource, ShouldInvalidateCallback shouldInvalidateCallback) {
        long j2;
        ...
        setPlaceables(map);
        boolean m8434needsRemeasureNN6EwU = m8434needsRemeasureNN6EwU(j, compositionSource, shouldInvalidateCallback);
        if (this.lastProgressInInterpolation != f || ((((layoutInformationReceiver = getLayoutInformationReceiver()) == null || layoutInformationReceiver.getForcedWidth() != Integer.MIN_VALUE) && ((layoutInformationReceiver2 = getLayoutInformationReceiver()) == null || layoutInformationReceiver2.getForcedHeight() != Integer.MIN_VALUE)) || m8434needsRemeasureNN6EwU)) {
            j2 = j;
            motionMeasurer = this;
            motionMeasurer.m8435recalculateInterpolation36Wf7g4(j2, layoutDirection, constraintSet, constraintSet2, transitionImpl, list, i, f, m8434needsRemeasureNN6EwU);
        } else {
            j2 = j;
            motionMeasurer = this;
        }
        motionMeasurer.oldConstraints = Constraints.m7936boximpl(j2);
    ...
    private final boolean m8434needsRemeasureNN6EwU(long j, CompositionSource compositionSource, ShouldInvalidateCallback shouldInvalidateCallback) {
        ...
        if (constraints != null && shouldInvalidateCallback != null) {
            ...
            if (shouldInvalidateCallback.mo8374invokeN9IONVI(constraints.m7955unboximpl(), j)) {
                return true;
            }
        } else if ((Constraints.m7946getHasFixedHeightimpl(j) && !getState().sameFixedHeight(Constraints.m7948getMaxHeightimpl(j))) || (Constraints.m7947getHasFixedWidthimpl(j) && !getState().sameFixedWidth(Constraints.m7949getMaxWidthimpl(j)))) {
            return true;
        }
    ...
    private final void m8435recalculateInterpolation36Wf7g4(long j, LayoutDirection layoutDirection, ConstraintSet constraintSet, ConstraintSet constraintSet2, TransitionImpl transitionImpl, List<? extends Measurable> list, int i, float f, boolean z) {
        ...
        if (z) {
            ...
            state2.height(min2);
            getState().m8464setRootIncomingConstraintsBRTryo0(j);
            getState().setRtl(layoutDirection == LayoutDirection.Rtl);
            m8433measureConstraintSethBUhpc(i, constraintSet, list, j);
            this.transition.updateFrom(getRoot(), 0);
            m8433measureConstraintSethBUhpc(i, constraintSet2, list, j);
        ...
        while (true) {
            ...
            if (measurable != null && (interpolated = this.transition.getInterpolated(constraintWidget)) != null) {
                getPlaceables().put(measurable, measurable.mo6698measureBRTryo0(Constraints.Companion.m7959fixedJhjzzOo(interpolated.width(), interpolated.height())));
    ...
    public final void drawDebug(DrawScope drawScope, boolean z, boolean z2, boolean z3) {
        ...
        while (i < size) {
            ...
            if (z) {
                m8431drawFrameg2O1Hgs(drawScope, start, dashPathEffect, Color.Companion.m5307getBlue0d7_KjU());
                m8431drawFrameg2O1Hgs(drawScope, end, dashPathEffect, Color.Companion.m5307getBlue0d7_KjU());
                drawScope.getDrawContext().getTransform().translate(2.0f, 2.0f);
                try {
                    m8431drawFrameg2O1Hgs(drawScope, start, dashPathEffect, Color.Companion.m5317getWhite0d7_KjU());
                    widgetFrame = start;
                    m8431drawFrameg2O1Hgs(drawScope, end, dashPathEffect, Color.Companion.m5317getWhite0d7_KjU());
            ...
            drawPaths(drawScope, Size.m5105getWidthimpl(drawScope.mo5859getSizeNHjbRc()), Size.m5102getHeightimpl(drawScope.mo5859getSizeNHjbRc()), widgetFrame, z2, z3);
            i++;
    ...
    private final void m8432drawFrameDebugPE3pjmc(DrawScope drawScope, float f, float f2, WidgetFrame widgetFrame, WidgetFrame widgetFrame2, PathEffect pathEffect, long j) {
        m8431drawFrameg2O1Hgs(drawScope, widgetFrame, pathEffect, j);
        m8431drawFrameg2O1Hgs(drawScope, widgetFrame2, pathEffect, j);
        ...
        while (true) {
            ...
            Path.close();
            int i4 = i3;
            DrawScope.m5849drawPathLG529CI$default(drawScope, Path, j, 1.0f, new Stroke(3.0f, 0.0f, 0, 0, null, 30, null), null, 0, 48, null);
            if (i4 == i2) {
                return;
            }
            i3 = i4 + 1;
    ...
    private final void m8431drawFrameg2O1Hgs(DrawScope drawScope, WidgetFrame widgetFrame, PathEffect pathEffect, long j) {
        if (widgetFrame.isDefaultTransform()) {
            DrawScope.m5853drawRectnJ9OG0$default(drawScope, j, OffsetKt.Offset(widgetFrame.left, widgetFrame.top), SizeKt.Size(widgetFrame.width(), widgetFrame.height()), 0.0f, new Stroke(3.0f, 0.0f, 0, 0, pathEffect, 14, null), null, 0, 104, null);
            return;
        }
        ...
        DrawScope.m5845drawLineNGM6Ib0$default(drawScope, j, OffsetKt.Offset(fArr[0], fArr[1]), OffsetKt.Offset(fArr[2], fArr[3]), 3.0f, 0, pathEffect, 0.0f, null, 0, 464, null);
        DrawScope.m5845drawLineNGM6Ib0$default(drawScope, j, OffsetKt.Offset(fArr[2], fArr[3]), OffsetKt.Offset(fArr[4], fArr[5]), 3.0f, 0, pathEffect, 0.0f, null, 0, 464, null);
        DrawScope.m5845drawLineNGM6Ib0$default(drawScope, j, OffsetKt.Offset(fArr[4], fArr[5]), OffsetKt.Offset(fArr[6], fArr[7]), 3.0f, 0, pathEffect, 0.0f, null, 0, 464, null);
        DrawScope.m5845drawLineNGM6Ib0$default(drawScope, j, OffsetKt.Offset(fArr[6], fArr[7]), OffsetKt.Offset(fArr[0], fArr[1]), 3.0f, 0, pathEffect, 0.0f, null, 0, 464, null);
    }

    /* renamed from: getCustomColor-XeAY9LY  reason: not valid java name */
    public final long m8436getCustomColorXeAY9LY(String str, String str2, float f) {
    ...
    public static final float transition$lambda$1(Density density, float f) {
        return density.mo430toPx0680j_4(Dp.m7996constructorimpl(f));
    }
}
