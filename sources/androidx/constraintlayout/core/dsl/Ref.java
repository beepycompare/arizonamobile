...
public class Ref {
    ...
    public String toString() {
        ...
        if (!Float.isNaN(this.mPostMargin)) {
            sb.append(StringUtils.COMMA).append(!Float.isNaN(this.mWeight) ? this.mWeight : 0.0f).append(StringUtils.COMMA);
            sb.append(Float.isNaN(this.mPreMargin) ? 0.0f : this.mPreMargin).append(StringUtils.COMMA);
            sb.append(this.mPostMargin);
        } else if (!Float.isNaN(this.mPreMargin)) {
            sb.append(StringUtils.COMMA).append(Float.isNaN(this.mWeight) ? 0.0f : this.mWeight).append(StringUtils.COMMA);
            sb.append(this.mPreMargin);
        } else if (!Float.isNaN(this.mWeight)) {
            sb.append(StringUtils.COMMA).append(this.mWeight);
        }
        if (z) {
            sb.append("]");
        }
