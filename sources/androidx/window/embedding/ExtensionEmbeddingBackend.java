...
public final class ExtensionEmbeddingBackend implements EmbeddingBackend {
    ...
    public static final class Companion {
        ...
        private final EmbeddingInterfaceCompat initAndVerifyEmbeddingExtension(Context context) {
            ...
            try {
                if (isExtensionVersionSupported(Integer.valueOf(extensionVersion)) && EmbeddingCompat.Companion.isEmbeddingAvailable()) {
                    ClassLoader classLoader = EmbeddingBackend.class.getClassLoader();
                    if (classLoader != null) {
                        ActivityEmbeddingComponent embeddingComponent = EmbeddingCompat.Companion.embeddingComponent();
                        EmbeddingAdapter embeddingAdapter = new EmbeddingAdapter(new PredicateAdapter(classLoader));
                        embeddingCompat = new EmbeddingCompat(embeddingComponent, embeddingAdapter, new ConsumerAdapter(classLoader), context, extensionVersion >= 8 ? new OverlayControllerImpl(embeddingComponent, embeddingAdapter) : null, extensionVersion >= 6 ? new ActivityWindowInfoCallbackController(embeddingComponent) : null);
                    } else {
