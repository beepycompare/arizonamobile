...
public final class AppUpdateManagerKtxKt {
    ...
    */
    ...
    public static final <T> Object runTask(Task<T> task, final Function0<Unit> function0, Continuation<? super T> continuation) {
        ...
        if (task.isComplete()) {
            if (task.isSuccessful()) {
                cancellableContinuationImpl2.resumeWith(Result.m9084constructorimpl(task.getResult()));
            } else {
                Exception exception = task.getException();
                Intrinsics.checkNotNull(exception);
                cancellableContinuationImpl2.resumeWith(Result.m9084constructorimpl(ResultKt.createFailure(exception)));
            }
        } else {
    ...
    public static final <E> boolean tryOffer(SendChannel<? super E> sendChannel, E e) {
        ...
        return ChannelResult.m10620isSuccessimpl(sendChannel.mo8127trySendJP2dKIU(e));
    }
}
