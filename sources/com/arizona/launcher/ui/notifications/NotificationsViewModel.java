...
public final class NotificationsViewModel extends ViewModel implements INotificationViewModel {
    ...
    private final boolean useMockNotifications;
    ...
    public NotificationsViewModel(NotificationsRepository notificationsRepository, SchedulersFacade schedulers) {
        ...
        this.notificationsLiveDataCompose = Transformations.map(mutableLiveData, new Function1() { // from class: com.arizona.launcher.ui.notifications.NotificationsViewModel$$ExternalSyntheticLambda0
            ...
            public final Object invoke(Object obj) {
                return NotificationsViewModel.notificationsLiveDataCompose$lambda$0((List) obj);
            }
        });
    ...
    */
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public final void loadMockNotifications() {
        long currentTimeMillis = System.currentTimeMillis();
        int i = 0;
        List<ArizonaNotification> listOf = CollectionsKt.listOf((Object[]) new ArizonaNotification[]{new ArizonaNotification(new Date(currentTimeMillis - SessionHotsetPolicy.DEFAULT_HIDDEN_TTL_MS), "Обновление лаунчера", "Проверь новый экран уведомлений и общую композицию карточек.", resourceUri(R.drawable.setup_image), false), new ArizonaNotification(new Date(currentTimeMillis - 7200000), "Игровой магазин", "Витрина с акциями и бонусами теперь доступна прямо из лаунчера.", resourceUri(R.drawable.ic_btn_shop), false), new ArizonaNotification(new Date(currentTimeMillis - 93600000), "Новости проекта", "Фоновый баннер и длинный текст нужны здесь только для визуальной проверки.", resourceUri(R.drawable.news_background), true), new ArizonaNotification(new Date(currentTimeMillis - 172800000), "Главный экран проекта", "Карточка с фоновым баннером помогает проверить кроп и читаемость текста.", resourceUri(R.drawable.arizona_backgournd_home), true)});
        this._notificationsLiveData.postValue(listOf);
        MutableLiveData<Integer> mutableLiveData = this._notificationsCountData;
        List<ArizonaNotification> list = listOf;
        if (!(list instanceof Collection) || !list.isEmpty()) {
            for (ArizonaNotification arizonaNotification : list) {
                if (!arizonaNotification.isViewed() && (i = i + 1) < 0) {
    ...
    private final String resourceUri(int i) {
        return "android.resource://com.arizona21.game/" + i;
    }
