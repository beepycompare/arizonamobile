...
public final class SingleFrom<T> extends Single<T> {
    ...
    public void subscribe(SingleObserver<T> downstream) {
        ...
        try {
            ...
            m9921constructorimpl = Result.m9921constructorimpl(this.source.invoke());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.Companion;
            m9921constructorimpl = Result.m9921constructorimpl(ResultKt.createFailure(th));
        }
        if (Result.m9928isSuccessimpl(m9921constructorimpl) && !simpleDisposable.isDisposed()) {
            downstream.onSuccess(m9921constructorimpl);
        }
        Throwable m9924exceptionOrNullimpl = Result.m9924exceptionOrNullimpl(m9921constructorimpl);
        if (m9924exceptionOrNullimpl == null || simpleDisposable.isDisposed()) {
            return;
        }
        downstream.onError(m9924exceptionOrNullimpl);
    }
}
