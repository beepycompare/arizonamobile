...
public final class SingleFrom<T> extends Single<T> {
    ...
    public void subscribe(SingleObserver<T> downstream) {
        ...
        try {
            ...
            m9915constructorimpl = Result.m9915constructorimpl(this.source.invoke());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.Companion;
            m9915constructorimpl = Result.m9915constructorimpl(ResultKt.createFailure(th));
        }
        if (Result.m9922isSuccessimpl(m9915constructorimpl) && !simpleDisposable.isDisposed()) {
            downstream.onSuccess(m9915constructorimpl);
        }
        Throwable m9918exceptionOrNullimpl = Result.m9918exceptionOrNullimpl(m9915constructorimpl);
        if (m9918exceptionOrNullimpl == null || simpleDisposable.isDisposed()) {
            return;
        }
        downstream.onError(m9918exceptionOrNullimpl);
    }
}
