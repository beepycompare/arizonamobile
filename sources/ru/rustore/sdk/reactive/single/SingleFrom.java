...
public final class SingleFrom<T> extends Single<T> {
    ...
    public void subscribe(SingleObserver<T> downstream) {
        ...
        try {
            ...
            m8500constructorimpl = Result.m8500constructorimpl(this.source.invoke());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.Companion;
            m8500constructorimpl = Result.m8500constructorimpl(ResultKt.createFailure(th));
        }
        if (Result.m8507isSuccessimpl(m8500constructorimpl) && !simpleDisposable.isDisposed()) {
            downstream.onSuccess(m8500constructorimpl);
        }
        Throwable m8503exceptionOrNullimpl = Result.m8503exceptionOrNullimpl(m8500constructorimpl);
        if (m8503exceptionOrNullimpl == null || simpleDisposable.isDisposed()) {
            return;
        }
        downstream.onError(m8503exceptionOrNullimpl);
    }
}
