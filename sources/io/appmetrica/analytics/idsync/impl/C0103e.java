...
public final class C0103e implements ProtobufConverter {
    ...
    public final m fromModel(IdSyncConfig idSyncConfig) {
        m mVar = new m();
        mVar.f430a = idSyncConfig.getEnabled();
        l lVar = new l();
        lVar.f429a = idSyncConfig.getLaunchDelay();
        int size = idSyncConfig.getRequests().size();
        k[] kVarArr = new k[size];
        for (int i = 0; i < size; i++) {
            kVarArr[i] = this.f422a.fromModel(idSyncConfig.getRequests().get(i));
        }
        lVar.b = kVarArr;
        mVar.b = lVar;
        return mVar;
    ...
    public final IdSyncConfig toModel(m mVar) {
        ...
        boolean z = mVar.f430a;
        long j = lVar.f429a;
        k[] kVarArr = lVar.b;
        ArrayList arrayList = new ArrayList(kVarArr.length);
        for (k kVar : kVarArr) {
            arrayList.add(this.f422a.toModel(kVar));
        }
