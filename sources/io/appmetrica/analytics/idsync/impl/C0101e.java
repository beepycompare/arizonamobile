...
public final class C0101e implements ProtobufConverter {
    ...
    public final m fromModel(IdSyncConfig idSyncConfig) {
        m mVar = new m();
        mVar.f479a = idSyncConfig.getEnabled();
        l lVar = new l();
        lVar.f478a = idSyncConfig.getLaunchDelay();
        int size = idSyncConfig.getRequests().size();
        k[] kVarArr = new k[size];
        for (int i = 0; i < size; i++) {
            kVarArr[i] = this.f471a.fromModel(idSyncConfig.getRequests().get(i));
        }
        lVar.b = kVarArr;
        mVar.b = lVar;
        return mVar;
    ...
    public final IdSyncConfig toModel(m mVar) {
        ...
        boolean z = mVar.f479a;
        long j = lVar.f478a;
        k[] kVarArr = lVar.b;
        ArrayList arrayList = new ArrayList(kVarArr.length);
        for (k kVar : kVarArr) {
            arrayList.add(this.f471a.toModel(kVar));
        }
