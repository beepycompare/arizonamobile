package io.appmetrica.analytics.screenshot.impl;

import kotlin.jvm.internal.Intrinsics;
/* renamed from: io.appmetrica.analytics.screenshot.impl.i  reason: case insensitive filesystem */
/* loaded from: classes4.dex */
public final class C0779i {

    /* renamed from: a  reason: collision with root package name */
    public final boolean f1324a;

    public C0779i(boolean z) {
        this.f1324a = z;
    }

    public final boolean equals(Object obj) {
        if (this == obj) {
            return true;
        }
        if (Intrinsics.areEqual(C0779i.class, obj != null ? obj.getClass() : null)) {
            if (obj != null) {
                return this.f1324a == ((C0779i) obj).f1324a;
            }
            throw new NullPointerException("null cannot be cast to non-null type io.appmetrica.analytics.screenshot.impl.config.client.model.ClientSideApiCaptorConfig");
        }
        return false;
    }

    public final int hashCode() {
        return Boolean.hashCode(this.f1324a);
    }

    public final String toString() {
        return "ClientSideApiCaptorConfig(enabled=" + this.f1324a + ')';
    }

    public C0779i(C0793x c0793x) {
        this(c0793x.a());
    }
}
