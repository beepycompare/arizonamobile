package io.appmetrica.analytics.impl;

import io.appmetrica.analytics.coreutils.internal.time.SystemTimeProvider;
import io.appmetrica.analytics.coreutils.internal.time.TimeProvider;
import java.util.concurrent.TimeUnit;
/* renamed from: io.appmetrica.analytics.impl.qk  reason: case insensitive filesystem */
/* loaded from: classes4.dex */
public final class C0543qk {

    /* renamed from: a  reason: collision with root package name */
    public volatile long f1054a;
    public C0612tf b;
    public TimeProvider c;

    public static C0543qk c() {
        return AbstractC0518pk.f1038a;
    }

    public final synchronized long a() {
        return (System.currentTimeMillis() / 1000) + this.f1054a;
    }

    public final synchronized void b() {
        this.b.d(false);
        this.b.b();
    }

    public final synchronized long d() {
        return this.f1054a;
    }

    public final synchronized void e() {
        a(Na.F.x(), new SystemTimeProvider());
    }

    public final synchronized boolean f() {
        return this.b.b(true);
    }

    public final synchronized void a(long j, Long l) {
        this.f1054a = (j - this.c.currentTimeMillis()) / 1000;
        boolean z = true;
        if (this.b.b(true)) {
            if (l != null) {
                long abs = Math.abs(j - this.c.currentTimeMillis());
                C0612tf c0612tf = this.b;
                if (abs <= TimeUnit.SECONDS.toMillis(l.longValue())) {
                    z = false;
                }
                c0612tf.d(z);
            } else {
                this.b.d(false);
            }
        }
        this.b.d(this.f1054a);
        this.b.b();
    }

    public final void a(C0612tf c0612tf, TimeProvider timeProvider) {
        this.b = c0612tf;
        this.f1054a = c0612tf.a(0);
        this.c = timeProvider;
    }
}
