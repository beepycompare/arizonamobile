package io.appmetrica.analytics.impl;
/* renamed from: io.appmetrica.analytics.impl.hh  reason: case insensitive filesystem */
/* loaded from: classes4.dex */
public final class C0319hh extends AbstractC0193ch {
    public final ro b;

    public C0319hh(C0580s5 c0580s5) {
        this(c0580s5, c0580s5.t());
    }

    @Override // io.appmetrica.analytics.impl.AbstractC0193ch
    public final boolean a(C0333i6 c0333i6) {
        C0580s5 c0580s5 = this.f785a;
        if (this.b.c()) {
            return false;
        }
        if (!this.b.d()) {
            C9 c9 = c0580s5.n;
            c9.c.b(C0333i6.a(c0333i6, EnumC0636ub.EVENT_TYPE_FIRST_ACTIVATION));
        }
        ro roVar = this.b;
        synchronized (roVar) {
            so soVar = roVar.f1033a;
            soVar.a(soVar.a().put("first_event_done", true));
        }
        return false;
    }

    public C0319hh(C0580s5 c0580s5, ro roVar) {
        super(c0580s5);
        this.b = roVar;
    }
}
