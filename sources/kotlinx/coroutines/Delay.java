...
public interface Delay {
    ...
    public static final class DefaultImpls {
        ...
        public static Object delay(Delay delay, long j, Continuation<? super Unit> continuation) {
            return Delay.super.delay(j, continuation);
        }

        @Deprecated
        public static DisposableHandle invokeOnTimeout(Delay delay, long j, Runnable runnable, CoroutineContext coroutineContext) {
            return Delay.super.invokeOnTimeout(j, runnable, coroutineContext);
        }
    }

    @Deprecated(level = DeprecationLevel.ERROR, message = "Deprecated without replacement as an internal method never intended for public use")
    static /* synthetic */ Object delay$suspendImpl(Delay delay, long j, Continuation<? super Unit> continuation) {
        if (j <= 0) {
            return Unit.INSTANCE;
        }
        CancellableContinuationImpl cancellableContinuationImpl = new CancellableContinuationImpl(IntrinsicsKt.intercepted(continuation), 1);
        cancellableContinuationImpl.initCancellability();
        delay.mo11513scheduleResumeAfterDelay(j, cancellableContinuationImpl);
