...
public final class AwaitKt$awaitOne$2$1<T> implements Subscriber<T> {
    ...
    public void onNext(T t) {
        ...
        if (subscription == null) {
            ...
            if (i == 1 || i == 2) {
                ...
                this.$cont.resumeWith(Result.m9896constructorimpl(t));
            } else if (i != 3 && i != 4 && i != 5) {
                ...
                if ((this.$mode == Mode.SINGLE || this.$mode == Mode.SINGLE_OR_DEFAULT) && this.seenValue) {
                    ...
                    if (this.$cont.isActive()) {
                        ...
                        this.$cont.resumeWith(Result.m9896constructorimpl(ResultKt.createFailure(new IllegalArgumentException("More than one onNext value for " + this.$mode))));
                        return;
                    }
                    return;
                }
    ...
    public void onComplete() {
        if (tryEnterTerminalState("onComplete")) {
            ...
            if (z) {
                ...
                Result.Companion companion = Result.Companion;
                this.$cont.resumeWith(Result.m9896constructorimpl(this.value));
            } else if (mode == Mode.FIRST_OR_DEFAULT || this.$mode == Mode.SINGLE_OR_DEFAULT) {
                Result.Companion companion2 = Result.Companion;
                this.$cont.resumeWith(Result.m9896constructorimpl(this.$default));
            } else if (this.$cont.isActive()) {
                Result.Companion companion3 = Result.Companion;
                this.$cont.resumeWith(Result.m9896constructorimpl(ResultKt.createFailure(new NoSuchElementException("No value received via onNext for " + this.$mode))));
            }
        }
    }
    ...
    public void onError(Throwable th) {
        if (tryEnterTerminalState("onError")) {
            Result.Companion companion = Result.Companion;
            this.$cont.resumeWith(Result.m9896constructorimpl(ResultKt.createFailure(th)));
        }
    }
