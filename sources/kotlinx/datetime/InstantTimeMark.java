...
final class InstantTimeMark implements ComparableTimeMark {
    ...
    public long mo11562elapsedNowUwyO8pc() {
        return m11832saturatingDiff3nIYWDw(this.clock.now(), this.instant);
    }

    @Override // kotlin.time.TimeMark
    /* renamed from: plus-LRDsOJo */
    public ComparableTimeMark mo11565plusLRDsOJo(long j) {
        return new InstantTimeMark(m11831saturatingAddHG0u8IE(this.instant, j), this.clock);
    }

    @Override // kotlin.time.TimeMark
    /* renamed from: minus-LRDsOJo */
    public ComparableTimeMark mo11563minusLRDsOJo(long j) {
        return new InstantTimeMark(m11831saturatingAddHG0u8IE(this.instant, Duration.m11616unaryMinusUwyO8pc(j)), this.clock);
    }
    ...
    public long mo11564minusUwyO8pc(ComparableTimeMark other) {
        ...
        if (other instanceof InstantTimeMark) {
            ...
            if (Intrinsics.areEqual(instantTimeMark.clock, this.clock)) {
                return m11832saturatingDiff3nIYWDw(this.instant, instantTimeMark.instant);
            }
        }
    ...
    private final boolean isSaturated(Instant instant) {
        ...
        if (!Intrinsics.areEqual(instant.m11672plusLRDsOJo(DurationKt.toDuration(1, DurationUnit.SECONDS)), instant)) {
    ...
    private final Instant m11831saturatingAddHG0u8IE(Instant instant, long j) {
        if (isSaturated(instant)) {
            if (Duration.m11597isInfiniteimpl(j)) {
                if (Duration.m11599isPositiveimpl(j) != (instant.compareTo(Instant.Companion.getDISTANT_FUTURE()) >= 0)) {
                    throw new IllegalArgumentException("Summing infinities of different signs");
                }
            }
            return instant;
        }
        return instant.m11672plusLRDsOJo(j);
    }

    /* renamed from: saturatingDiff-3nIYWDw  reason: not valid java name */
    private final long m11832saturatingDiff3nIYWDw(Instant instant, Instant instant2) {
        ...
        if (isSaturated(instant) || isSaturated(instant2)) {
            return Duration.m11602timesUwyO8pc(instant.m11671minusUwyO8pc(instant2), Double.POSITIVE_INFINITY);
        }
        return instant.m11671minusUwyO8pc(instant2);
    }
}
