...
final class InstantTimeMark implements ComparableTimeMark {
    ...
    public long mo11284elapsedNowUwyO8pc() {
        return m11538saturatingDiff3nIYWDw(this.clock.now(), this.instant);
    }

    @Override // kotlin.time.ComparableTimeMark, kotlin.time.TimeMark
    /* renamed from: plus-LRDsOJo */
    public ComparableTimeMark mo11287plusLRDsOJo(long j) {
        return new InstantTimeMark(m11537saturatingAddHG0u8IE(this.instant, j), this.clock);
    }

    @Override // kotlin.time.ComparableTimeMark, kotlin.time.TimeMark
    /* renamed from: minus-LRDsOJo */
    public ComparableTimeMark mo11285minusLRDsOJo(long j) {
        return new InstantTimeMark(m11537saturatingAddHG0u8IE(this.instant, Duration.m11340unaryMinusUwyO8pc(j)), this.clock);
    }
    ...
    public long mo11286minusUwyO8pc(ComparableTimeMark other) {
        ...
        if (other instanceof InstantTimeMark) {
            ...
            if (Intrinsics.areEqual(instantTimeMark.clock, this.clock)) {
                return m11538saturatingDiff3nIYWDw(this.instant, instantTimeMark.instant);
            }
        }
    ...
    private final boolean isSaturated(Instant instant) {
        ...
        if (!Intrinsics.areEqual(instant.m11401plusLRDsOJo(DurationKt.toDuration(1, DurationUnit.SECONDS)), instant)) {
    ...
    private final Instant m11537saturatingAddHG0u8IE(Instant instant, long j) {
        if (isSaturated(instant)) {
            if (Duration.m11321isInfiniteimpl(j)) {
                if (Duration.m11323isPositiveimpl(j) != (instant.compareTo(Instant.Companion.getDISTANT_FUTURE()) >= 0)) {
                    throw new IllegalArgumentException("Summing infinities of different signs");
                }
            }
            return instant;
        }
        return instant.m11401plusLRDsOJo(j);
    }

    /* renamed from: saturatingDiff-3nIYWDw  reason: not valid java name */
    private final long m11538saturatingDiff3nIYWDw(Instant instant, Instant instant2) {
        ...
        if (isSaturated(instant) || isSaturated(instant2)) {
            return Duration.m11326timesUwyO8pc(instant.m11400minusUwyO8pc(instant2), Double.POSITIVE_INFINITY);
        }
        return instant.m11400minusUwyO8pc(instant2);
    }
}
