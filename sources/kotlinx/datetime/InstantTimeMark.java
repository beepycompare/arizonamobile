...
final class InstantTimeMark implements ComparableTimeMark {
    ...
    public long mo11285elapsedNowUwyO8pc() {
        return m11539saturatingDiff3nIYWDw(this.clock.now(), this.instant);
    }

    @Override // kotlin.time.ComparableTimeMark, kotlin.time.TimeMark
    /* renamed from: plus-LRDsOJo */
    public ComparableTimeMark mo11288plusLRDsOJo(long j) {
        return new InstantTimeMark(m11538saturatingAddHG0u8IE(this.instant, j), this.clock);
    }

    @Override // kotlin.time.ComparableTimeMark, kotlin.time.TimeMark
    /* renamed from: minus-LRDsOJo */
    public ComparableTimeMark mo11286minusLRDsOJo(long j) {
        return new InstantTimeMark(m11538saturatingAddHG0u8IE(this.instant, Duration.m11341unaryMinusUwyO8pc(j)), this.clock);
    }
    ...
    public long mo11287minusUwyO8pc(ComparableTimeMark other) {
        ...
        if (other instanceof InstantTimeMark) {
            ...
            if (Intrinsics.areEqual(instantTimeMark.clock, this.clock)) {
                return m11539saturatingDiff3nIYWDw(this.instant, instantTimeMark.instant);
            }
        }
    ...
    private final boolean isSaturated(Instant instant) {
        ...
        if (!Intrinsics.areEqual(instant.m11402plusLRDsOJo(DurationKt.toDuration(1, DurationUnit.SECONDS)), instant)) {
    ...
    private final Instant m11538saturatingAddHG0u8IE(Instant instant, long j) {
        if (isSaturated(instant)) {
            if (Duration.m11322isInfiniteimpl(j)) {
                if (Duration.m11324isPositiveimpl(j) != (instant.compareTo(Instant.Companion.getDISTANT_FUTURE()) >= 0)) {
                    throw new IllegalArgumentException("Summing infinities of different signs");
                }
            }
            return instant;
        }
        return instant.m11402plusLRDsOJo(j);
    }

    /* renamed from: saturatingDiff-3nIYWDw  reason: not valid java name */
    private final long m11539saturatingDiff3nIYWDw(Instant instant, Instant instant2) {
        ...
        if (isSaturated(instant) || isSaturated(instant2)) {
            return Duration.m11327timesUwyO8pc(instant.m11401minusUwyO8pc(instant2), Double.POSITIVE_INFINITY);
        }
        return instant.m11401minusUwyO8pc(instant2);
    }
}
