...
public abstract class AbstractDateTimeFormat<T, U extends Copyable<U>> implements DateTimeFormat<T> {
    ...
    public T parse(CharSequence input) {
        ...
        try {
            try {
                return (T) valueFromIntermediate(Parser.m11515matchimpl$default(Parser.m11510constructorimpl(getActualFormat().parser()), charSequence, getEmptyIntermediate(), 0, 4, null));
    ...
    public T parseOrNull(CharSequence input) {
        Intrinsics.checkNotNullParameter(input, "input");
        Copyable m11517matchOrNullimpl$default = Parser.m11517matchOrNullimpl$default(Parser.m11510constructorimpl(getActualFormat().parser()), input, getEmptyIntermediate(), 0, 4, null);
        if (m11517matchOrNullimpl$default != null) {
            return (T) valueFromIntermediateOrNull(m11517matchOrNullimpl$default);
        }
        return null;
    }
}
