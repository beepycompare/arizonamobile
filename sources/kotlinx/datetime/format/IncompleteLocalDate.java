    ...
    public final LocalDate toLocalDate() {
        ...
        if (dayOfYear == null) {
            ...
            if (getMonthNumber() != null) {
                int number = MonthKt.getNumber(plus.m11543getMonth());
                Integer monthNumber = getMonthNumber();
                if (monthNumber == null || number != monthNumber.intValue()) {
                    throw new DateTimeFormatException("Can not create a LocalDate from the given input: the day of year is " + dayOfYear + ", which is " + plus.m11543getMonth() + ", but " + getMonthNumber() + " was specified as the month number");
                }
            }
            if (getDay() != null) {
                int day = plus.getDay();
                ...
                if (day2 == null || day != day2.intValue()) {
                    throw new DateTimeFormatException("Can not create a LocalDate from the given input: the day of year is " + dayOfYear + ", which is the day " + plus.getDay() + " of " + plus.m11543getMonth() + ", but " + getDay() + " was specified as the day of month");
                }
            }
            localDate = plus;
        }
        Integer dayOfWeek = getDayOfWeek();
        if (dayOfWeek == null || (intValue = dayOfWeek.intValue()) == DayOfWeekKt.getIsoDayNumber(localDate.m11542getDayOfWeek())) {
            return localDate;
        }
        throw new DateTimeFormatException("Can not create a LocalDate from the given input: the day of week is " + DayOfWeekKt.m11538DayOfWeek(intValue) + " but the date is " + localDate + ", which is a " + localDate.m11542getDayOfWeek());
    }

    public final void populateFrom(LocalDate date) {
        Intrinsics.checkNotNullParameter(date, "date");
        setYear(Integer.valueOf(date.getYear()));
        setMonthNumber(Integer.valueOf(MonthKt.getNumber(date.m11543getMonth())));
        setDay(Integer.valueOf(date.getDay()));
        setDayOfWeek(Integer.valueOf(DayOfWeekKt.getIsoDayNumber(date.m11542getDayOfWeek())));
